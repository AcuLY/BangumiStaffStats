...
Define the closed integrated-development acceptance harness, its canonical formal verdict, and the exact non-equivalent CI/remote lifecycle closure authorized for the completed development rewrite.
...
`contracts/acceptance/**`, the exact reviewed active and dated archive paths
for `produce-development-artifacts`,
`close-release-readiness-identities`, and this acceptance change, plus the
exact main-spec files updated by those archives. Every protected product and
artifact implementation blob and executable mode SHALL be identical between
the two trees. Both exact dependencies SHALL be archived in the
harness/control tree, no other active change besides this acceptance change
SHALL exist there, and neither checkout SHALL be dirty.
