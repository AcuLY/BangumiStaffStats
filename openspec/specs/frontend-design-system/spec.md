...
The implementation SHALL change source for a reproducible acceptance failure
or an explicitly accepted design delta, and SHALL use
public component APIs, semantic markup/classes, and documented tokens rather
than private component-library DOM or internals. Validation SHALL follow the
AGENTS.md proportionality rule: bounded visual/copy corrections need focused
evidence and diff hygiene, while substantial changes need affected full gates.
Regression tests SHALL cover reachable behavior risks rather than duplicate
low-impact style values.
...
- **THEN** the smallest owned layer is corrected and verified proportionally; substantial changes also rerun the complete affected-component gate
...
The root `DESIGN.md` SHALL remain authoritative for `.impeccable/design.json`.
An explicitly requested documentation refresh MAY update the sidecar from
accepted design decisions and their verified implementation. It SHALL preserve
the existing visual identity, record the documentation baseline, and SHALL NOT
mark outstanding implementation, bundle, browser or release gates complete.
Final design-system release acceptance SHALL still require the applicable
code, bundle, browser and vertical lifecycle gates.

#### Scenario: Sidecar is regenerated

- **WHEN** accepted design information is synchronized into the sidecar
- **THEN** the sidecar is valid schema version 2 extension metadata derived from current `DESIGN.md` and verified implementation, with required narrative copied verbatim and 5–10 representative self-contained public-token `ds-` snippets
...
#### Scenario: Documentation refresh with outstanding acceptance

- **WHEN** documentation is refreshed while a dependency, code, bundle, browser, or review gate is not complete
- **THEN** that gate remains incomplete, historical verification evidence is retained, and the sidecar update is not reported as release acceptance
