...
- [ ] 1.1 Record clean HEAD/index/worktree, active changes, exact owned-file
  ...
  protected drift.
- [ ] 1.2 In Rankings, Candidates, Person Detail, Partners, and Co-Star, replace
...
- [ ] 1.3 Extend the existing CI policy test to enumerate exactly Catalog plus
...
- [ ] 2.1 Run the focused CI-policy test and syntax-check all six verifiers.
  Prove the five package manifests and locks and every schema/golden/OpenAPI
  path remain byte-identical.
- [ ] 2.2 With `frontend/node_modules` absent, independently run exact
  ...
  roots.
- [ ] 2.3 Run all Contracts artifact tests, strict validation for this change
...
- [ ] 3.1 Main agent audits the import-only verifier diff and regression test,
  independently repeats focused verification, marks completed tasks, and
  commits the reviewed implementation.
- [ ] 3.2 Main agent synchronizes and archives this change, verifies one active
  acceptance change remains, creates a clean acceptance-free Product
  candidate, rebuilds source-bound development artifacts, and resumes formal
  integrated acceptance. No release, deployment, or operations claim follows
  from this repair.
