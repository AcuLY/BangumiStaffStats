...
- [x] 1.3 Mark the four Status rows approved and commit only `openspec/config.yaml` plus this change's five artifacts as `docs(openspec): approve Node 24 toolchain`.

## 2. Standard NVM update — main

- [x] 2.1 Recheck the planning commit, index/worktree envelope, NVM/default state, and official version immediately before mutation.
- [x] 2.2 In a clean shell source `/Users/luca/.nvm/nvm.sh`, run `nvm install 24.18.0`, then `nvm alias default 24.18.0`; do not use global migration flags.
- [x] 2.3 Use `apply_patch` to add `.nvmrc` and `.node-version` with exact `24.18.0\n`.

## 3. Acceptance — main

- [x] 3.1 Verify installed/default/project/fresh-shell Node `v24.18.0` and bundled npm; record the inherited-process restart caveat.
- [x] 3.2 Run targeted/all strict OpenSpec validation and doctor with absolute Node `24.18.0`, plus `git diff --check` and exact status/diff checks.
- [x] 3.3 Verify startup files, older runtimes/aliases, manifests/locks, application files, remotes, and production state were not changed.
- [x] 3.4 Update the four Status rows truthfully, sync/archive the capability, stage only the accepted envelope, and commit `chore(toolchain): standardize Node 24.18.0` without push.
