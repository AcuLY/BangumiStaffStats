...
| Stop/rollback conditions | Stop before mutation if an owner needs a non-listed path, new dependency, external/remote state, production path, activation behavior, or incompatible producer change. On apply rejection, discard only this change's uncommitted writable-path edits. A pre-replace status-write failure must preserve the prior file byte-for-byte; failure of the parent-directory fsync after replace is reported without falsely claiming that the old bytes remain or that durability was confirmed. An already published inactive Archive is never deleted or relabeled as activated. |
