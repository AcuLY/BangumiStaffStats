...
|---|---|
| Status | investigated: complete; specified: approved; implemented: no; verified: main semantic/dependency/library review and strict validation passed |
| Owner | Backend implementation owner; main agent reviews and accepts. |
| Writable paths | `backend/internal/query/**`, `backend/go.mod`, `backend/go.sum`, `backend/internal/architecture/dependencies_test.go`, `backend/scripts/check.sh`, `backend/README.md`, Backend task markers. |
| Read-only protected inputs | Shared contracts/goldens, accepted Archive/catalog code and data, generated HTTP wire, guides/root specs, other code/changes/state. |
| Deletion complement | None. |
| Mutable refs | None during apply. |
| Consumes | Shared query authority, corrected Archive v1, published Store, typed catalog plans, immutable collection overlay, query-domain goldens. |
| Produces | Effective Query/digest and deterministic raw result/identity sets. |
| Dependencies | `define-shared-query-wire`, `correct-archive-subject-semantics`, `implement-backend-archive-consumer`, `derive-position-catalog-and-cast`. |
| Deliverables | Normalizer, Archive fact loader, set algebra, dependency/check/doc updates, tests. |
| Acceptance | Existing contract vectors, all domain goldens, integration/isolation/cancel/determinism/race/full gates. |
| Non-goals | Statistics, series, view projection, HTTP, collection fetch, cache, frontend, schema/producer changes. |
| Operations deferred | Production data/activation/services/monitoring/migration/release/deploy. |
| Stop/rollback conditions | Stop on unmet dependency, authority/path drift, inferred subject semantics, personal leakage, writable SQL, or failed gate; publish no partial result and preserve accepted state. |

Dependency direction SHALL be `query -> archive`; query MUST NOT import
transport, app, cache, external collection-client, or generated wire packages.
