...
|---|---|
| Status | specified for main review; implementation pending |
| Owner | Backend implementation subagent; main-agent acceptance. |
| Writable paths | `backend/internal/imageproxy/**` and the exact integration/test files declared by the proposal. |
| Read-only protected inputs | Contracts, Archive, updater, frontend, other changes/specs, refs/remotes, hosts, production. |
| Deletion complement | None. |
| Mutable refs | None during apply. |
| Consumes | Accepted backend HTTP/runtime and observability capabilities. |
| Produces | One bounded same-origin Bangumi image route and upstream adapter. |
| Dependencies | Accepted `implement-backend-http-and-observability`. |
| Deliverables | Production Go code and focused tests. |
| Acceptance | Security, route, streaming, cache, cancellation, race, vet, build, scope, and strict-spec gates. |
| Non-goals | Wildcard proxy, UI size policy, collection API, CDN, deploy, operations. |
| Operations deferred | Production cache/network tuning, rollout, monitoring deployment. |
| Stop/rollback conditions | Stop on caller-controlled upstream location or required out-of-scope writes. |
