...
|---|---|
| Status | investigated: complete; specified: approved; implemented: yes; verified: main diff audit plus clean install, zero-vulnerability audit, 94 tests, generation/type/build/artifact, browser, Impeccable finish, path, strict OpenSpec, and residue gates passed; committed: determined by containing Git history; pushed/released/deployed: no |
| Owner | One frontend subagent implements; main specifies/audits/accepts/finalizes; a separate Impeccable reviewer performs read-only rendered review. |
| Writable paths | `frontend/**`, own task markers, and exact accepted OpenSpec lifecycle paths. |
| Protected inputs | Shared contracts/specs, Node/Impeccable/PRODUCT/DESIGN/surface/sidecar context, sibling paths, editor state, external/remote/production state. |
| Operations deferred | Push, PR, tag, release, deploy, production routing/config/secrets, migration, and legacy deletion. |
