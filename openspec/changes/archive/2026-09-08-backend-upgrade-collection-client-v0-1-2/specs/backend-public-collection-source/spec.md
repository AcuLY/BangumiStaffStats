...
Status: specified; Owner: primary/backend. Writable paths, protected inputs, deletion complement, mutable refs (None), consumes/produces, dependencies, deliverables, acceptance, non-goals, operations deferred and stop/rollback conditions are the exact boundary table in proposal.md; no additional scope is granted.
