...
- [x] 4.2 Run `bash operations/test/runtime.sh`, strict OpenSpec validation, shell syntax, exact one-line pin diff, `git diff --check`, and zero-P0/P1 review; commit/push the admission change and verify remote equality. The local canonical runtime wrapper stopped before execution because `jq` is unavailable; shell syntax, strict OpenSpec, one-line `1:1` pin diff, product-diff closure, and diff hygiene passed directly. Admission commit `80ed50c236da778cb35e1833aeeb823262829e94` was pushed with remote equality, and the exact GitHub bundle job subsequently ran the canonical runtime wrapper with reviewed prerequisites and succeeded.

## 5. Exact AMD64 artifact

- [x] 5.1 Manually dispatch `ci.yml` for the repair branch, require the exact admission commit’s complete Product job and `operations-preview / build one linux/amd64 operations bundle` job to succeed, and record run/job/artifact identities. Manual run `30732403889` at `80ed50c236da778cb35e1833aeeb823262829e94` succeeded: Product job `91454998145`, bundle job `91456043495`, artifact `8828550683` named `operations-preview-80ed50c236da778cb35e1833aeeb823262829e94`.
- [x] 5.2 Download the one-day `operations-preview-<source-revision>` artifact into an exact run-owned local root; verify closed inventory, `SHA256SUMS`, `build.json` source revision/tree/platform/image names, frontend `/v2/` entry assets, and non-fixture deployment metadata before any host write. The nine-file artifact was downloaded to `D:/Luca/Code/.tmp-bgmss-deploy/run-30732403889`, all eight payload checksums passed, inventory was exact, and `build.json` bound revision `80ed50c236da778cb35e1833aeeb823262829e94`, tree `057a852716250ebe3b0a564e994f091da5fd7d02`, `linux/amd64`, and matching API/updater image names. The frontend archive retained the deployed `/v2/assets/**` entry/deferred files; its bundled minimal Archive remains validation-only and will not replace production data.

## 6. Production preflight and transaction

- [x] 6.1 On `myserver`, bind `/srv/bgmss-v2`, project `bgmss-v2`, ports `18080`/`19090`, Prometheus pin, current/previous revisions and refs, current real Archive data version, operation lock availability, release/incoming-path absence, disk capacity, container identities, `/livez`, `/readyz`, catalog, metrics, Prometheus, recent logs, public legacy `/`, and public `/v2/`; stop on drift. Preflight bound current `a714fe7e865c93a4c2f4ed4b9dcbf2ef9ea7286a`, previous `1150a94e77dcd7e269bf58a96f5df93ba451882a`, data version `dv1-9d794033f12b8bcd60d8c890115a76ca52060ae13b357b3c32e036f94bb67888`, free capacity 52.8 GB, available lock, absent candidate paths, healthy checks/Prometheus, unchanged legacy containers/root, and no recent panic/fatal/5xx evidence.
- [x] 6.2 Transfer only the admitted bundle to absent `/srv/bgmss-v2/incoming/run-<run-id>/`, reverify checksum/metadata on-host, and invoke the existing `/srv/bgmss-v2/operations/bin/deploy` with unchanged root/project/ports/Prometheus/profile inputs. Require the new revision current and the preimage retained as previous; do not run updater or mutate Nginx/data. The exact run `30732403889` bundle passed all on-host checksums and metadata assertions. The existing transaction deployed `80ed50c236da778cb35e1833aeeb823262829e94`, reported API ready and switched frontend last, while retaining `a714fe7e865c93a4c2f4ed4b9dcbf2ef9ea7286a` as previous.

## 7. Live acceptance and cleanup

- [x] 7.1 Re-run the `lucay126` personal rankings/detail flow and assert raw HTTP 200, schema-valid `warningCodes: []`, correct person identity, and no `null`; verify a second request/cache hit remains valid. Two live detail requests for person `1913` 石原立也 returned the same data version and allocated empty warning arrays with distinct request IDs; neither response contained `warningCodes:null`.
- [x] 7.2 Verify in a fresh browser that personal ranking selection renders the inspector with no “人物详情加载失败” or console error; close the task-opened browser afterward. The fresh browser query `导演 · lucay126 · 动画 · 已完成 + 进行中` returned 258 people/448 subjects; selecting 石原立也 rendered the 9-work inspector. DOM inspection found the detail copy, no failure copy, one successful person-detail resource, and zero console/JavaScript errors; the browser was returned to `about:blank`.
- [x] 7.3 Run the host operations check; verify current revision/image label, current/previous refs, `/livez`, `/readyz`, catalog/data-version agreement, metrics, Prometheus target, recent logs without new panic/5xx, public legacy root, V2 entry/deferred asset/API, and unchanged Archive/Nginx/legacy identities. Current/image label are `80ed50c236da778cb35e1833aeeb823262829e94`, previous is `a714fe7e865c93a4c2f4ed4b9dcbf2ef9ea7286a`; checks and Prometheus passed, protected Archive/Nginx hashes and legacy container identities stayed fixed, public root/V2/deferred asset hashes stayed fixed, catalog data version matched, and recent error scan was empty.
- [x] 7.4 Remove only the exact run-owned incoming transfer and local downloaded artifact after acceptance; confirm no operation lock/temp candidate residue and retain the previous release for rollback. Removed only `/srv/bgmss-v2/incoming/run-30732403889` and `D:/Luca/Code/.tmp-bgmss-deploy/run-30732403889`; both are absent, the lock is available, no candidate temp release exists, current remains `80ed50c236da778cb35e1833aeeb823262829e94`, and previous `a714fe7e865c93a4c2f4ed4b9dcbf2ef9ea7286a` remains present.

## 8. Specification lifecycle

- [x] 8.1 Record exact Product/admission/manual-run/artifact/deploy/browser/host evidence in this task file, sync the delta into `openspec/specs/backend-person-detail-api/spec.md`, mark all tasks honestly, archive the change, and run strict validation for all specs. Evidence was recorded above, the delta was synced without semantic drift, the change was archived as `2026-08-02-repair-person-detail-warning-wire`, and all 53 root specs passed strict validation.
