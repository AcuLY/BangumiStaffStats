...
| Read-only protected inputs | `PRODUCT.md`, `DESIGN.md`, `openspec/config.yaml`, existing root specs, all formal-development guides/decisions, Impeccable, fixed oracle `644b7748674e553f863d0ffd61d029f86fdc0717`, all sibling query bytes including exact Query-owned root `.gitignore`, externally created untracked `.vscode/settings.json` at SHA-256 `048f53e6ca01ac583b48784cd2f6f7d248e0534849955b144e75f017f73188a3`, and concurrent spec-only `openspec/changes/bootstrap-backend-runtime/**`, `openspec/changes/bootstrap-updater-runtime/**`, and `openspec/changes/bootstrap-frontend-foundation/**` |
| Deletion complement | Every path outside this owner’s two apply roots; no pre-existing/unlisted file in them may be deleted/renamed. Sibling Query product paths including root `.gitignore`, the three exact Wave 1B spec-only planning directories, and exact sealed untracked `.vscode/settings.json` are tolerated dirty/foreign state only and never writable by this apply owner; the paired finalizer may only stage already accepted Query bytes |
| Mutable refs | Accepted checkpoints are `c7f868e2861e8fea250f033c27538ecf793bacad`, `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c`, `52af379d48d7c29a479b59a4670ae07d73b1c0fb`, and editor-state amendment `dcadeb0cb3092583b4c368767a0777cecc719596`. This `tasks.md` checkbox state only; index and Git refs remain immutable. Finalization may advance that branch only through the exact accepted combined Wave 1A commit |
...
| Stop/rollback conditions | Stop on root/branch/HEAD/index/approval/sibling/path/tool/vector drift, dirty-state drift outside the exact sibling/Wave 1B spec-only planning/editor allowances, non-OpenSpec planning output, or editor-state drift. Preserve both candidates; never reset hard, checkout rollback, clean, `git add -A`, broad recursive delete, stage/commit/archive while sibling runs, amend/rebase, or write outside declared paths |
...
- [x] 4.1 Before sealing, reverify root/branch/exact planning HEAD, empty index, approved artifact bytes, no Archive ephemeral/ignored output, and the exact changed-path union. The only tolerated non-Archive changes are the sibling’s exact task/apply roots plus exact sealed untracked `.vscode/settings.json`.
- [x] 4.2 Produce a deterministic Archive candidate manifest containing every owned physical path, Git status, mode, byte size, SHA-256/Git blob where applicable, schema/tool versions, local Python/Node/Go/SQLite feasibility versions, every exact command/result, and explicit states for investigated/specified/implemented/verified/committed/pushed/released/deployed. Store it only in the agent handoff, not as a repository file.
- [x] 4.3 Stop with all Archive outputs unstaged and no ref mutation. The main agent SHALL perform read-only path/schema/vector/SQLite/tooling acceptance and return zero P0/P1 findings plus the exact accepted candidate seal before this owner marks the implementation candidate accepted.
- [x] 4.4 After acceptance, rerun the seal without changing accepted bytes, record `implemented=yes`, `verified=yes`, `committed=no`, `pushed=no`, `released=no`, `deployed=no`, and stop permanently. Do not stage, commit, archive, or modify the sibling candidate.

After 4.4 completed, the main agent authorized spec-only Wave 1B planning under exactly `openspec/changes/bootstrap-backend-runtime/**`, `openspec/changes/bootstrap-updater-runtime/**`, and `openspec/changes/bootstrap-frontend-foundation/**`. This does not reopen the Archive product candidate or authorize this stopped owner to act. The three directories are read-only foreign state, may contain only OpenSpec artifacts, and must remain byte-identical and excluded throughout Wave 1A finalization.
...
## 5. Paired finalization protocol — outside this change's completion checkbox set

- 5.1 After both apply agents are stopped, every product checkbox in both changes is complete, and both candidates are independently accepted, provide the finalization subagent with the exact planning HEAD, two active-change seals, two candidate path/hash inventories, exact allowed combined union, exact phase subject `feat(contracts): establish wave 1 shared contracts`, and the requirement to read/follow `openspec-archive-change` before mutation.
- 5.2 Require the finalization subagent alone to re-prove empty index and exact combined state including accepted Query-owned root `.gitignore`, plus the three exact spec-only Wave 1B planning directories and unchanged sealed untracked `.vscode/settings.json`; validate both active change IDs; synchronize/archive only `define-archive-manifest-contract` and `define-shared-query-wire`; explicitly stage only their accepted contract outputs (including root `.gitignore`), exact active-change deletions, two generated archive directories, and two synchronized root specs, never Wave 1B planning paths or `.vscode/**`.
- 5.3 Require the finalizer to validate both synchronized root capability IDs plus repository-wide OpenSpec/doctor, verify recorded successful product evidence and byte-identical candidate seals without recreating cleaned disposable roots, run strict/cached/unstaged/path/hash/archive-root parity checks, and stop for final main-agent read-only acceptance. After that acceptance only, the same finalization subagent creates the exact-subject commit and proves a clean index, no worktree dirt except the three exact spec-only Wave 1B planning directories plus exact sealed untracked `.vscode/settings.json`, correct single parent/delta, both root specs strict-valid, and final states `committed=yes`, `pushed=no`, `released=no`, `deployed=no`; it does not amend, squash, push, tag, release, deploy, or activate.

The Archive apply owner MUST NOT execute this finalization protocol. It is intentionally not represented as checkboxes that would require mutation after the active change moves or its commit exists. Finalization paths do not broaden its product writable paths; they are the two approved OpenSpec archive/sync outputs and exact accepted combined contract union. All operations, actual `current.json`, full data acquisition/build, and runtime implementation remain deferred.
