...
| Writable paths | Exactly the planning and Frontend apply paths listed in `proposal.md`; no parent-directory implication. |
| Read-only protected inputs | Authorities, contracts, other changes/tasks, undeclared frontend files, other owners, refs/remotes, external state, and production listed in `proposal.md`. |
| Deletion complement | None. |
| Mutable refs | None. |
| Consumes | Accepted `frontend-foundation`, `contracts-query-wire`, and exited `contracts-catalog-api`; `PRODUCT.md` and `DESIGN.md`. |
| Produces | Query shell routes/UI, catalog consumer, shared query state, and typed operation coordination. |
...
| Deliverables | Source, generated catalog adapter, tests, browser evidence, and scoped documentation. |
| Acceptance | Strict catalog/query decoding, state-machine tests, frontend quality gates, desktop/mobile visual/console checks, and strict OpenSpec/diff checks. |
| Non-goals | Ranking/detail/candidate/partners/co-star result UI, selection, statistics, series, sort algorithms, backend/contracts/updater work, or shipped fixtures. |
| Operations deferred | Production configuration, release, deployment, migration, monitoring, and cutover. |
| Stop/rollback conditions | Stop on dependency/contract/path drift, fixture leakage, protected writes, or failed gates; remove only owned disposable output. |

## ADDED Requirements

### Requirement: One query shell SHALL own routes and shared query state

The formal SPA SHALL expose `/ranking` and `/co-star` as shell modes with one
`QueryDraft`, one immutable last-successful `AppliedQuery`, one ordered
`positionKeys` array, and one monotonic `queryRevision`. Operation/path SHALL
not enter the shared query signature. Editing Draft or switching modes SHALL
not apply, reset, or fork shared state; only a successful semantically new
application SHALL advance revision.
...
`/` and `/index.html` SHALL replace to `/ranking` while preserving safe query
parameters; `?user=` SHALL prefill only personal Draft. On first document load,
the shell SHALL consume at most one valid v1 share fragment through the same
ordinary application service and then remove the fragment by history
replacement. Invalid share SHALL remove the fragment, show a structured local
error, and start no request or automatic `?user=` fallback.
...
updates SHALL not start another request. The Header SHALL retain one share
action immediately beside the mode switch at every viewport. It SHALL remain
visibly disabled when no Applied Query exists and otherwise serialize only the
current mode's last-successful Applied Query plus accepted operation input/view
into the existing versioned fragment wire. Dirty Draft, pending attempts,
responses, request IDs, revision, dataVersion, digests, refresh flags, theme,
and transient UI state SHALL not enter the link.
...
- **WHEN** the user edits Draft and switches between ranking and co-star
- **THEN** Applied Query, queryRevision, ordered PositionKeys, and current-revision resources SHALL remain unchanged
- **AND** the edited Draft SHALL remain available in both modes
...
- **WHEN** one valid v1 share and `?user=other` are both present
- **THEN** the share SHALL be applied at most once through ordinary query application
- **AND** the fragment SHALL be removed and `?user=` SHALL not trigger another request
...
- **THEN** the share action SHALL encode the current visible result's last-successful Applied Query and accepted operation state
...
- **THEN** the same read-only selectable link SHALL remain available in the adjacent fallback popover
