...
| Owner | Contracts owner writes and verifies the OpenAPI and closed goldens first; Backend is a read-only consumer only after main-agent acceptance. |
| Writable paths | Exactly `contracts/openapi/openapi.yaml`, `contracts/goldens/api/catalog/**`, and Contracts task markers in this change. |
| Read-only protected inputs | All other Contracts paths, catalog/query/Archive schemas and goldens, updater/backend/frontend code, root specs, authorities, changes/tasks, refs/remotes, external state, and production. |
| Deletion complement | None; existing OpenAPI components and every accepted contract/golden remain present and no requirement is weakened. |
| Mutable refs | None; no stage, commit, sync/archive, ref mutation, push, tag, release, or deploy. |
| Consumes | Exited `derive-position-catalog-and-cast`, accepted shared query/error wire, PRODUCT catalog semantics, and accepted HTTP transport/envelope behavior. |
| Produces | The authoritative catalog operation/components in OpenAPI and a closed language-neutral API-golden corpus for all producers/consumers. |
| Dependencies | Exactly `derive-position-catalog-and-cast`, `implement-backend-archive-consumer`, and `implement-backend-http-and-observability`; all must have exited before apply. |
| Deliverables | OpenAPI operation/components; indexed success/error/invalid/dormant cases; strict verifier and generation evidence below the owned golden root. |
| Acceptance | OpenAPI 3.1 validation, closed inventory/digests, exact DTO and ordering, error/header scenarios, Go generation/replay, strict OpenSpec/diff/residue gates. |
| Non-goals | Go implementation, query result DTOs, statistics, UI, active staff-set policy, Archive/updater edits, collection access, operations, or deployment. |
| Operations deferred | Activation/reload, production roots, monitoring, release, deployment, migration, and cutover. |
| Stop/rollback conditions | Stop on dependency/owner/path drift, need for another shared schema, ambiguous upstream catalog semantics, unclosed evidence, or generation mismatch. Remove only new owned disposable output. |
