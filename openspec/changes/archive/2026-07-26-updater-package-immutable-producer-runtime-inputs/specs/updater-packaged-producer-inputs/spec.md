...
| Field | Contract |
|---|---|
| Status | New artifact runtime input capability; not released or deployed. |
| Owner | Updater. |
| Writable paths | Updater build/Docker/test paths declared by this change. |
| Read-only protected inputs | Contracts producer manifest/records, catalog pair, product source/locks, other components and external state. |
| Deletion complement | None beyond owned ignored output cleanup. |
| Mutable refs | None during apply. |
| Consumes | Contracts producer-runtime manifest and governed catalog pair. |
| Produces | Immutable source-free producer input subtree and metadata. |
| Dependencies | Accepted `contracts-producer-runtime-inputs` and Updater artifact runtime. |
| Deliverables | Native/OCI packaged inputs, evidence, verification and smoke. |
| Acceptance | Exact attested copy, byte/mode/path parity, embedded contract/catalog smoke, repeated artifacts, residue. |
| Non-goals | Authority or producer semantic changes, archive-smoke, actual produce/acquisition, scheduler, release/deploy. |
| Operations deferred | External mounts/paths, release assembly, timer, activation and cutover. |
| Stop/rollback conditions | Fail closed on authority/identity/path/type/mode/digest drift, broadened/partial copy, nondeterminism, or external mutation. |
