...
| Writable paths | Planning: `openspec/changes/define-shared-query-wire/**`. Apply: `contracts/openapi/**`, `contracts/schemas/query/**`, `contracts/goldens/query/**`, exact root `.gitignore`, and own task-checkbox markers |
| Read-only protected inputs | `PRODUCT.md`; `DESIGN.md`; `openspec/config.yaml`; `openspec/specs/contracts-rewrite-baseline/spec.md`; `tmp-formal-development/formal-development-master-plan.md`; `tmp-formal-development/data-logic-implementation-guide.md`; `tmp-formal-development/backend-development-implementation-guide.md`; `tmp-formal-development/decisions/prototype-data-logic-audit.md`; `.impeccable/**`; oracle `644b7748674e553f863d0ffd61d029f86fdc0717`, especially `frontend/src/workbench/types.ts`, `frontend/src/workbench/components/QueryWorkspace.vue`, `frontend/src/workbench/composables/useWorkbench.ts`, `frontend/public/workbench-data/co-star-snapshot.json`, and `frontend/public/workbench-data/position-data.json`; externally created untracked `.vscode/settings.json` at SHA-256 `048f53e6ca01ac583b48784cd2f6f7d248e0534849955b144e75f017f73188a3`; concurrent spec-only directories `openspec/changes/bootstrap-backend-runtime/**`, `openspec/changes/bootstrap-updater-runtime/**`, and `openspec/changes/bootstrap-frontend-foundation/**` |
| Deletion complement | Every path outside the three contract roots, own task markers, and explicitly authorized root `.gitignore` rewrite. No existing contract path may be deleted/moved, and no other root/framework/editor/sibling path may be altered or hidden. Exact sealed untracked `.vscode/settings.json` is tolerated protected foreign state, not writable output |
| Mutable refs | Accepted checkpoints are `c7f868e2861e8fea250f033c27538ecf793bacad`, `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c`, `52af379d48d7c29a479b59a4670ae07d73b1c0fb`, and editor-state amendment `dcadeb0cb3092583b4c368767a0777cecc719596`. Product writes remain permitted only inside the exact apply Writable paths; within OpenSpec control state, only own task-checkbox markers may change. Every Git ref/index remains immutable until the accepted combined finalization commit; no amend/rebase/tag/push |
...
| Acceptance | Exact paired-checkpoint/index/path gates; strict OpenSpec; strict JSON Schema compilation; OpenAPI lint; all vectors and idempotence/digest/share-byte checks; deterministic TS generation from authority; exact A/B codegen-only sanitized source projections and independently expanded public graph; Redocly telemetry/config/network isolation; byte-identical fully dereferenced JSON bundles with zero ref/resource/anchor keywords and canonical public-schema equivalence; exact Go `models,skip-prune` generation from projection A with all 17 public names represented by non-header declarations plus syntax/format/compile smoke; exact root ignore positive/negative matrix and physical cleanup of all cache/temp/generated paths |
...
| Stop/rollback conditions | Stop on missing approval, branch/HEAD/dependency/protected-input mismatch, dirty-state mismatch outside the exact apply/sibling/spec-only planning/editor allowances, editor-state drift, write outside owned paths, unlocked tool, failed lint/vector/codegen, or P0/P1 finding. Preserve evidence and use explicit path-scoped correction only; never reset-hard, checkout rollback, git clean, broad recursive deletion, or history rewrite |

Acceptance SHALL distinguish stable output evidence from cold-cache progress and SHALL prove cleanup safety before mutation. Generated bytes, hashes, declarations, and deterministic stdout remain exact. Candidate-success Go stderr admission uses named policy `go-download-progress-v1` plus an exact sorted allowlist of complete dependency pairs; `gofmt` stderr is empty. The stable manifest SHALL store that policy and allowlist rather than an observed stderr byte count, while the handoff stores each candidate-success child's actual bytes/result and keeps historical recovery attempts separate. The one real cleanup SHALL follow a passing synthetic safety test and SHALL remove only the six exact roots through fail-closed `lstat`/containment checks, no-follow handling of internal links, directory-only permission repair, bounded retry, per-root absence proof, and exact `removed`/`alreadyAbsent` evidence.
...
The corrected sandboxed command subsequently exposed that `oapi-codegen/v2@v2.8.0` does not resolve the authority's external standalone JSON Schema references without import mappings. Because those files are not separate OpenAPI packages, import mapping SHALL remain forbidden. Plain Redocly bundling SHALL also remain forbidden because retaining absolute `$id` while relocating schemas changes Draft 2020-12 resource bases; unsanitized dereferencing duplicates those resource IDs. Redocly telemetry SHALL be disabled and independently network-denied.

The reviewed replacement SHALL use `verify.mjs` to prove a closed, resolvable, sibling-free DAG with no dynamic/recursive refs, anchors, or nested resource/dialect identifiers; create exact `.tmp/codegen-a/**` and `.tmp/codegen-b/**` source-relative trees; copy OpenAPI bytes unchanged; and remove only each of seven temporary schema roots' one exact `$id` and one exact `$schema`, with all other data deep-equal. Each tree SHALL contain exact `redocly.yaml` bytes `{}\n`, no lint-ignore file, the copied OpenAPI, all seven sanitized schemas, and its JSON bundle. Locked Redocly SHALL run directly under exact Node 24.16.0 with clean owned HOME/TMPDIR, `REDOCLY_TELEMETRY=off`, explicit config, and an exact sandbox profile containing `(version 1)`, `(allow default)`, and `(deny network*)`; source lint SHALL additionally use exact `--extends recommended`; bundle commands SHALL retain the empty config and exact full-dereference/JSON/naming/conflict/unused/URL-ref flags; and the runs SHALL produce byte-identical A/B bundles. A sandbox denial or nonzero child SHALL fail; evidence SHALL assert no successful network operation and SHALL NOT treat absence of a denial message as proof that no attempt occurred. Both bundles SHALL contain none of `$ref`, `$dynamicRef`, `$recursiveRef`, `$id`, `$schema`, `$anchor`, or `$dynamicAnchor`; preserve exact source OpenAPI metadata/zero paths/17 public names; and expose one recorded helper inventory. Their response-name sets and each response's `description`, `x-error-codes`, and media-type keys SHALL exactly equal authority; each `application/json.schema` SHALL canonical-deep-equal the verifier's independent expansion of `ErrorEnvelopeV1`; no other response field is allowed; and the dereferenced response objects SHALL NOT be required to deep-equal source objects containing `$ref`. Their 17 public schemas SHALL canonical-deep-equal the verifier's independent cycle-safe expansion, while authority and projection validators SHALL agree over every golden on acceptance, stable classification, and instance path. Only bundle A may feed exact Go `models,skip-prune`. All projection/bundle evidence SHALL be recorded and all transient bytes removed before handoff.
...
The second correction SHALL be accepted as exact HEAD `52af379d48d7c29a479b59a4670ae07d73b1c0fb`. Before either apply owner resumes, untracked regular non-symlink `.vscode/settings.json` SHALL have appeared as external editor-owned state. Its directory inventory SHALL contain exactly that file, its SHA-256 SHALL be `048f53e6ca01ac583b48784cd2f6f7d248e0534849955b144e75f017f73188a3`, and it SHALL have no index membership. Both owners' product counts/seals and checked sets SHALL remain exactly the second-boundary values above.

The delegated workspace-state amendment staged exactly the same eight OpenSpec artifact paths, had sole parent `52af379d48d7c29a479b59a4670ae07d73b1c0fb`, and was accepted as `dcadeb0cb3092583b4c368767a0777cecc719596` with exact subject `docs(openspec): record wave 1 external editor state`. It staged or modified no `.vscode/**`, product, cache, or temp byte and ran no product-writing command.

#### Scenario: External editor state is preserved and tolerated

- **WHEN** the workspace-state amendment and each resumed preflight inspect `.vscode/**`
- **THEN** exactly the one untracked non-symlink file and hash above SHALL be present outside the combined implementation envelope
- **AND** no owner SHALL write, delete, hide, stage, commit, or claim it; any path/content/status drift SHALL stop work without repair
...
Wave 1A apply SHALL share one approved planning/correction chain with `define-archive-manifest-contract`. The sibling envelope is exactly `contracts/schemas/archive/**`, `contracts/goldens/archive/**`, and checkbox markers in `openspec/changes/define-archive-manifest-contract/tasks.md`. Exact sealed untracked `.vscode/settings.json` remains protected. Concurrent spec-only planning MAY appear under exactly `openspec/changes/bootstrap-backend-runtime/**`, `openspec/changes/bootstrap-updater-runtime/**`, and `openspec/changes/bootstrap-frontend-foundation/**`; these directories SHALL contain only OpenSpec artifacts and are read-only foreign state for Query, not inputs or dependency-completion evidence. This change's agent MAY observe concurrent sibling and planning mutations only in those exact envelopes, SHALL snapshot and never write/target/stage/commit/archive/clean those paths or `.vscode/**`, SHALL keep the Git index empty, SHALL use path-scoped checks, and SHALL stop as an unstaged candidate. Only after both Wave 1A candidates pass main-agent acceptance MAY one finalization subagent stage the exact Wave 1A outputs, archive both active changes, and create one combined Wave 1A phase commit; it SHALL exclude all Wave 1B planning paths.
...
- **WHEN** `openapi-typescript@7.13.0` reads `contracts/openapi/openapi.yaml`, the repo verifier plus isolated locked Redocly create two proven codegen-only projections, and `oapi-codegen/v2@v2.8.0` reads bundle A
- **THEN** TypeScript generation and exact Go `-generate models,skip-prune` generation complete without unresolved references or schema-level errors from the same authoritative referenced JSON Schemas
...
- **AND** no projection, temporary bundle, or generated Go/TypeScript consumer file is committed by this change
