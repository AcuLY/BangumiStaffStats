...
- Generate query DTOs at the HTTP adapter boundary with
  `oapi-codegen/v2@v2.8.0` from a deterministic backend-local sanitized and
  dereferenced projection of the shared authority.
...
- `contracts-query-wire`: Allow formal consumers to use a compatible
  types-only generator directly or a deterministic disposable projection while
  preserving one shared authority and exact component coverage.
...
| Writable paths | Planning: this six-file change. Apply: `backend/**` and task checkboxes. Lifecycle: exact archive plus `openspec/specs/backend-runtime-foundation/spec.md` and the modified `openspec/specs/contracts-query-wire/spec.md`. |
...
| Dependencies | Accepted Wave 1A contracts and locked query tooling, Node/Impeccable baseline, Go `1.26.5`, Redocly `2.40.0`, and `oapi-codegen/v2@v2.8.0`. |
| Acceptance | Exact module/tool versions; authority-preserving projection and generated drift checks; lifecycle/architecture/query/Archive tests; `go test -race`, vet, build, strict OpenSpec, Git boundary checks; no disposable residue. |
