...
backend/scripts/prepare-query-wire.mjs
...
Go development tool is
`github.com/oapi-codegen/oapi-codegen/v2/cmd/oapi-codegen` at v2.8.0;
production runtime code remains standard-library-only. Query generation also
reuses the accepted contract package lock in a copied backend-local temporary
tool root to obtain exact Redocly `2.40.0`; it adds no backend runtime
dependency and never installs below `contracts/**`.
...
`oapi-codegen/v2@v2.8.0` cannot resolve the authority's external JSON Schema
references directly. `prepare-query-wire.mjs` therefore reads the authority
without mutation, copies the OpenAPI document and seven query schemas below
`backend/.tmp/`, removes only each schema root's `$id` and `$schema`, and proves
the exact 17-component inventory. The generation script copies the accepted
contract `package.json` and lock into a separate backend-local temporary tool
root, performs a locked install with backend-local npm cache, and uses exact
Redocly `2.40.0` to fully dereference the sanitized projection.

`go tool oapi-codegen -generate models,skip-prune -package wire` consumes only
that temporary bundle and creates
`internal/httpapi/wire/query_wire.gen.go`. `skip-prune` is required because the
contract currently has components but no paths. Check mode builds a fresh
projection below `backend/.tmp/`, verifies its bundle bytes and hash against the
shared manifest evidence, and byte-compares the generated Go file.

No generated handler/client, private schema copy, normalizer, digest, cache, or
business result DTO is admitted. No projection, copied tool root,
`node_modules`, or npm cache survives cleanup, and every tracked file below
`contracts/**` remains byte-identical.
