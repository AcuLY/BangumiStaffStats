...
| Owner | One Frontend implementation owner; a separate read-only Impeccable finish reviewer; main-agent acceptance. |
| Writable paths | Proven corrective hunks under `frontend/src/**`, corresponding `frontend/tests/**`, the four exact checker/config paths declared in the proposal when required by a failing gate, exact `.impeccable/design.json` after final acceptance gates, and task markers. |
| Read-only protected inputs | Product/design authorities, oracle, all other `.impeccable/**`, formal guides, contracts/backend/updater/generated DTOs and generators, package manifests/lockfile, other OpenSpec artifacts, refs/remotes, external repositories, and operations state. |
| Deletion complement | Delete only tests newly created here; restore existing files by owned hunk and sidecar by exact preimage. |
| Mutable refs | None during apply; remote refs are protected. |
| Consumes | Archived ranking/co-star verticals, accepted frontend capabilities, oracle, current `PRODUCT.md`/`DESIGN.md`, production contracts/APIs, and Impeccable tooling. |
| Produces | Oracle-compatible final presentation, proven regression fixes, clean production bundle, and current schema-v2 Impeccable sidecar. |
...
| Deliverables | Oracle evidence, design-system/SafeImage checks, production artifact evidence, focused tests/fixes, and validated sidecar. |
| Acceptance | Every scenario below passes against `frontend/dist`; independent review reports zero P0/P1. |
| Non-goals | Redesign, aesthetic polish, new behavior/API/statistics/state/dependency, private library DOM, protected-authority edits, or unrelated cleanup. |
...
| Stop/rollback conditions | Stop on unmet archive/ownership gates, unproven or unapproved differences, protected paths, undeclared behavior/dependency, redesign, or premature sidecar work. Revert only owned hunks/new tests and restore the sidecar preimage. |
