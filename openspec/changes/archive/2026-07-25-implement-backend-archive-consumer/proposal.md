...
- Reject `null` for every required manifest/source field and every
  `tableCounts`/`qualitySummary` value before typed Go decoding can turn it
  into a valid-looking zero value.
- Preserve JSON Schema 2020-12 integer semantics at the real Go boundary:
  zero-fraction decimal and exponent spellings such as `1.0` and `1e0` are
  exact integers, while fractional or out-of-safe-range values fail without
...
- Bind the approved archive root and every pooled VFS open to the validated
  filesystem identities, gate the final publication window on context
  cancellation, and drain active query rows before DB, VFS, and roots close.
- Keep readiness false and close every candidate handle on failure; close the
  published store once during application shutdown.
...
- `backend-runtime-foundation`: Admit the pinned SQLite runtime dependency,
  Archive-gated process lifecycle, and the package graph introduced by the
  consumer and its smoke command.
...
| Deliverables | Consumer package with fatal UTF-8, raw non-null, and exact JSON-Schema-integer contract decoding, exact generated-time/Unicode-scalar/surrogate validation, shared manifest-string vector execution, canonical actual-SQLite schema-object seal verification, root/per-open identity binding, final cancellation gates, and managed query-row draining; minimal startup wiring, candidate-smoke CLI, dependency/architecture/check updates, and tests. |
| Acceptance | Corrected shared valid/invalid goldens and all indexed manifest-string cases through the real Go decoder; exhaustive required-field null and integer-spelling mutations; invalid-UTF-8 pointer/manifest, weakened-definition/extra-object, root/VFS rebound, final-cancel including lock wait, active-row shutdown, path/write/concurrency/lifecycle cases; `go test ./...`, `go test -race ./...`, vet/build, dependency/inventory checks, strict change/all validation, and no residue. |
...
Official dependency research is read-only; this change touches no other repository or external mutable state. Final acceptance and lifecycle finalization remain blocked until the amended artifacts strictly validate and pass main-agent review.
