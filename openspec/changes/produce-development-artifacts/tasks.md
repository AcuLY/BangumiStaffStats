...
- [x] 2.1 Preflight group C by printing branch, HEAD, and
...
- [x] 2.2 In `contracts/artifacts/**`, implement and test closed versioned
...
- [x] 2.3 Publish the reviewed component-statement schema/fixture interface as
...
- [x] 3.1 Preflight group A by printing branch, HEAD, and
  `git status --short`; verify they match the main-agent record, this change is
...
- [x] 3.2 Implement `backend/Dockerfile` and `backend/build/**`: digest-pinned
...
- [x] 3.3 Emit and test the complete sorted SHA-256 inventory, deterministic
  ...
  drift.
- [x] 3.4 Run `cd backend && ./scripts/check.sh`, then the new Backend build
...
- [x] 4.1 Preflight group B by printing branch, HEAD, and
  `git status --short`; verify they match the main-agent record, this change is
...
- [x] 4.2 Implement `updater/Dockerfile` and `updater/build/**`: digest-pinned
  ...
  deployment behavior.
- [x] 4.3 Emit and test the complete sorted SHA-256 inventory, deterministic
...
- [x] 4.4 Run the exact Python 3.14.6/uv 0.11.32 frozen
...
- [x] 5.1 In only `frontend/build/**`, `frontend/package.json`,
  ...
  test.
- [x] 5.2 Run Node 24.18.0/npm 11.16.0 `npm ci` and `npm run check`, then the
  ...
  and `git diff --check`.
- [x] 5.3 Run the packaged static artifact from outside the source tree on a
...
- [x] 6.1 After groups A and B hand off owner-local outputs, validate all three
...
- [x] 6.2 Implement the Contracts artifact-only smoke coordinator in
  ...
  paths absent.
- [x] 6.3 Implement `.github/workflows/ci.yml` with `contents: read`, pinned
...
- [x] 6.4 Run the Contracts positive/negative suites, two full clean local
...
- [x] 7.1 Audit every implementation diff against the three leases, all
...
- [x] 7.2 Stage only the exact approved change/product paths into an isolated
...
- [x] 7.3 Prove generated artifact/cache directories are ignored and contain
  ...
  manifest.
- [x] 7.4 Record status fields independently: investigated, specified,
