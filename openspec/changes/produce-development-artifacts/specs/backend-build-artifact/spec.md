...
| Writable paths | `backend/Dockerfile` and `backend/build/**`; generated output only below ignored `backend/build/.tmp/**`. |
| Read-only protected inputs | Root authorities/oracle/OpenSpec outside this change; `contracts/**`; all Backend source, module, script, README, and config files outside the writable paths; all Updater/Frontend paths; external repositories/state. |
...
`backend/Dockerfile` SHALL be multi-stage and pin each base image by immutable
digest. Its runtime stage SHALL contain the API executable and only required
runtime trust/data files, SHALL contain no source, module cache, compiler, or
build tool, SHALL run as a non-root user, and SHALL accept Archive and optional
development status inputs only through explicit read-only mounts/arguments.
