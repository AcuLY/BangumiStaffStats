...
| Writable paths | This change; new `.github/workflows/operations-preview.yml`; new `operations/README.md`, `operations/.gitignore`, `operations/compose.yaml`, `operations/bin/**`, `operations/lib/**`, `operations/nginx/**`, `operations/prometheus/**`, and `operations/systemd/**`; one absent `/srv/bgmss-ops-validation-minimal-<run-id>/**` root on `myserver` during isolated validation. |
...
| Stop/rollback conditions | Stop before writes if branch/head, Actions, host architecture, Docker/Compose, path absence, project-name uniqueness, free loopback ports, disk, or legacy baseline checks fail. After owned writes, stop the exact project, restore any validation-local current/previous pointer or release env, remove only run-owned objects, and report any ambiguous residue without touching legacy state. |
