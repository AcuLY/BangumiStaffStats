...
| Mutable refs | Initial checkpoint is `c7f868e2861e8fea250f033c27538ecf793bacad`. One observed sealed correction may advance the branch once from it with subject `docs(openspec): approve wave 1 archive toolchain correction` and exactly both changes' proposal/design/spec/tasks, never product/cache bytes. After main-agent acceptance of that replacement checkpoint, parallel apply may change only task checkboxes and index/refs remain immutable. After both accepted candidates stop, one finalization subagent alone may create the combined phase commit |
...
The one authorized correction checkpoint stages exactly proposal, design, delta spec, and tasks for each of the two Wave 1A changes—the eight paths enumerated by the sibling Query design—and no product/cache path. A quiescent correction subagent re-proves both owners' seals and checkbox sets, creates the commit with sole parent `c7f868e2861e8fea250f033c27538ecf793bacad` and exact subject `docs(openspec): approve wave 1 archive toolchain correction`, and stops. The main agent performs read-only acceptance of the commit and unchanged paused candidates. Only then may the original Archive owner re-snapshot the replacement HEAD/sibling state and resume at task 2.2, applying the now-reviewed npm override. No package install, cleanup, code generation, verifier execution, fixture generation, or other product-writing command may run during the correction interval.
