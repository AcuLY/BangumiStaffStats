...
| Field | Declaration |
|---|---|
| Status | Planning only; apply blocked until both Wave 1A changes pass strict validation, are committed in one clean planning checkpoint, and receive explicit main-agent approval |
...
| Mutable refs | Pre-apply checkpoint: one delegated checkpoint subagent may advance `refs/heads/codex/formal-rewrite` once from parent `e5d67d7d74614b7a95da4a7887caa8e1f25bc307` with subject `docs(openspec): approve wave 1 shared contracts` and only the two approved change directories. Parallel apply: this `tasks.md` checkbox state only; index and Git refs immutable. Finalization: that branch only through the exact accepted combined Wave 1A commit |
...
- [ ] 1.1 Before the first write, verify the canonical root, `codex/formal-rewrite`, exact `WAVE1A_PLANNING_HEAD`, ordinary baseline ancestry, empty index, completely clean worktree/untracked/ignored state, and presence of both committed active change directories; stop without mutation on any mismatch.
- [ ] 1.2 Verify both changes are artifact-complete, `openspec validate --all --strict` and `openspec doctor --json` pass, this change’s proposal/design/spec/tasks bytes equal the main-agent-approved planning checkpoint, and the approval record has zero unresolved P0/P1 findings.
- [ ] 1.3 Record the HEAD tree/hash inventory of `openspec/changes/define-shared-query-wire/**` and the sibling’s four exact task/apply roots; prove they do not overlap this change’s `tasks.md`, `contracts/schemas/archive/**`, or `contracts/goldens/archive/**`.
- [ ] 1.4 End the group by rerunning the empty-index and clean-status checks plus strict validation; record `investigated=yes`, `specified=yes`, `implemented=no`, `verified=preflight-only`, `committed=planning-checkpoint-only`, `pushed=no`, `released=no`, `deployed=no`.
...
- [ ] 2.1 Before schema writes, reverify root/branch/exact HEAD, empty index, approval seal, and changed-path union. Allow only the exact sibling roots plus any already-created Archive roots/task checkbox changes; stop rather than restoring either owner’s bytes.
...
- [ ] 2.3 Create exactly the four approved tooling files. Pin `ajv@8.20.0` and `quicktype@26.0.0` as dev-only dependencies and lock the graph; implement strict schema/vector/hash verification and stdlib SQLite fixture construction without adding any other tool/dependency/file.
