...
- **Mutable refs**: the accepted initial checkpoint is `c7f868e2861e8fea250f033c27538ecf793bacad`. The observed quicktype/stream-json engine conflict authorizes exactly one sealed correction checkpoint after both apply owners stop: one delegated correction subagent may advance `refs/heads/codex/formal-rewrite` from that commit with exact subject `docs(openspec): approve wave 1 archive toolchain correction` and exactly proposal/design/spec/tasks for both Wave 1A changes, while product/cache output remains unstaged and byte-sealed. Apply resumes only after main-agent acceptance of the replacement checkpoint; then only task-checkbox transitions may change and the index/refs remain immutable. After both candidates pass acceptance and both apply agents stop, one finalization subagent alone may create the approved combined phase commit. No other ref is mutable.
