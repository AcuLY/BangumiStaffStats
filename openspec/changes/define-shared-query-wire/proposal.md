...
| Mutable refs | Before apply, one delegated checkpoint subagent may advance `refs/heads/codex/formal-rewrite` exactly once from parent `e5d67d7d74614b7a95da4a7887caa8e1f25bc307` with subject `docs(openspec): approve wave 1 shared contracts` and a delta containing only the two main-agent-approved Wave 1A change directories. During apply, only checkbox markers in `openspec/changes/define-shared-query-wire/tasks.md` may change and every Git ref/index remains immutable. After both paired candidates are accepted, one finalization subagent alone may advance that branch with the approved combined phase commit; no apply agent may stage or commit |
