...
| Mutable refs | Initial checkpoint `c7f868e2861e8fea250f033c27538ecf793bacad` and first correction `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c` are accepted. The observed endpoint-free Go-model pruning defect authorizes one second sealed correction: after both apply owners stop, one delegated correction subagent may advance `refs/heads/codex/formal-rewrite` once from `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c` with exact subject `docs(openspec): approve wave 1 query codegen correction` and exactly proposal/design/spec/tasks for both Wave 1A changes, while every product/cache/temp byte remains unstaged and sealed. Apply resumes only after main-agent acceptance of that replacement checkpoint; refs/index then remain immutable until one finalization subagent creates the approved combined phase commit. No apply owner may stage or commit |
...
| Acceptance | OpenSpec strict validation; JSON/YAML parse; JSON Schema draft 2020-12 strict compilation; OpenAPI 3.1 lint; every positive/negative/normalization/digest vector produces the declared outcome; repeated normalization, digest, and share encoding are byte-stable; `openapi-typescript@7.13.0` and exact `oapi-codegen/v2@v2.8.0 -generate models,skip-prune` generate deterministic non-header models from the same endpoint-free OpenAPI, with all 17 authoritative component-schema names present in the Go declaration inventory and temporary Go syntax/format/compile smoke passing; no generated output is committed; caches/temp/generated output stay under `contracts/goldens/query/**` and are absent from the candidate; paired-worktree path/index gates pass |
