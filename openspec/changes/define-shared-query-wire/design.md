...
| Read-only protected inputs | `PRODUCT.md`; `DESIGN.md`; `openspec/config.yaml`; `openspec/specs/contracts-rewrite-baseline/spec.md`; `tmp-formal-development/formal-development-master-plan.md`; `tmp-formal-development/data-logic-implementation-guide.md`; `tmp-formal-development/backend-development-implementation-guide.md`; `tmp-formal-development/decisions/prototype-data-logic-audit.md`; `.impeccable/**`; oracle commit and the five oracle paths listed above; externally created untracked `.vscode/settings.json` at SHA-256 `048f53e6ca01ac583b48784cd2f6f7d248e0534849955b144e75f017f73188a3` |
| Deletion complement | Empty; no tracked deletion, move, or rewrite is authorized outside the apply roots, and no existing path within them may be removed without an approved OpenSpec amendment. The exact untracked `.vscode/settings.json` is tolerated protected foreign state, never an implementation output or writable path |
| Mutable refs | Accepted checkpoints are initial `c7f868e2861e8fea250f033c27538ecf793bacad`, Archive toolchain correction `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c`, and Query codegen correction `52af379d48d7c29a479b59a4670ae07d73b1c0fb`. After both apply owners remain stopped, one workspace-state amendment may advance `refs/heads/codex/formal-rewrite` once from `52af379d48d7c29a479b59a4670ae07d73b1c0fb` with exact subject `docs(openspec): record wave 1 external editor state` and the same exact eight OpenSpec artifact paths, never product/cache/temp/editor output. Apply resumes only after main-agent acceptance; thereafter only task checkboxes may change and refs/index remain immutable until the accepted combined phase commit. No amend, rebase, tag, push, or other ref mutation |
...
| Acceptance | Strict OpenSpec; paired-checkpoint/index/path gates; JSON/YAML parse; strict schema compile; OpenAPI lint; all declared vectors; idempotent normalization and stable digest/share bytes; deterministic TS generation plus exact Go `models,skip-prune` generation whose non-header declaration inventory covers all 17 component schemas and passes syntax/format/compile smoke; all temp/cache/generated paths confined to and removed from the owned golden root; no generated consumer committed |
...
Both candidates, checked sets, and Query/Archive counts/seals remain exactly the second-boundary values above. One quiescent workspace-state amendment subagent may commit only the same eight OpenSpec paths from parent `52af379d48d7c29a479b59a4670ae07d73b1c0fb` with exact subject `docs(openspec): record wave 1 external editor state`, while the index excludes `.vscode/**` and every product/cache/temp byte. It runs no product-writing command. After main-agent acceptance, each original owner must require the file's exact untracked/non-symlink/hash state as a read-only tolerated status projection, reject any additional `.vscode/**` entry or drift, and then resume at Query 2.1 / Archive 2.5.
...
- both may observe only the exact sealed `.vscode/settings.json` outside the combined envelope and must reject but never repair any byte/state/path drift there;
...
| `oapi-codegen/v2@v2.8.0` | Go model generation feasibility from the same endpoint-free OpenAPI | Handwritten DTO (drift); generating handlers (premature transport ownership); default pruning (silently drops all currently unreferenced components) | Read-only Go module proxy input; no committed Go tool file/output | One pinned development download/cache; no runtime dependency yet; `skip-prune` is confined to this exact development check | Exact `go run ...@v2.8.0 -generate models,skip-prune` exits zero; output is non-header, deterministically hashed, declares every one of the 17 named component schemas, passes syntax/`gofmt`/temporary compile smoke, and writes no `backend/**` file |
