...
| Mutable refs | Accepted checkpoints are initial `c7f868e2861e8fea250f033c27538ecf793bacad` and Archive toolchain correction `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c`. After both apply owners stop and seal product/cache/temp bytes plus checkbox sets, one observed Query codegen correction may advance `refs/heads/codex/formal-rewrite` once from `ad5bf1a0fbca06aba1f7d2cff5be66e1d726701c` with exact subject `docs(openspec): approve wave 1 query codegen correction` and the same exact eight OpenSpec artifact paths, never product output. Apply resumes only after main-agent acceptance; thereafter only task checkboxes may change and refs/index remain immutable until the accepted combined phase commit. No amend, rebase, tag, push, or other ref mutation |
