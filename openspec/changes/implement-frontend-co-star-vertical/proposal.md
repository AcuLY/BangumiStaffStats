...
|---|---|
| Status | Investigated/specified/main-agent reviewed: complete. Group 1 is accepted at `9eae3216`, Group 2A at `f7233647`, and `expose-co-star` at `8b9d623a` with archive `a15073fb`. Group 2B feature implementation is committed at `77b335ae` and undergoing its final oracle-correction audit; App/share/shared/check integration remains protected until the main agent hands those paths to task 2.8. Nothing is pushed, released, or deployed. |
| Owner | One Frontend implementation agent owns apply; the main agent owns decisions, spec edits, audit, and acceptance. |
| Writable paths | `frontend/src/api/candidates.ts`, `frontend/src/api/partners.ts`, `frontend/src/api/coStar.ts`, `frontend/src/api/adapters/candidates.ts`, `frontend/src/api/adapters/partners.ts`, `frontend/src/api/adapters/coStar.ts`, the narrowly scoped read-only error-response metadata extension in `frontend/src/api/client.ts`, `frontend/src/features/co-star/**`, `frontend/src/features/query/coordinator.ts`, `frontend/src/features/query/share.ts`, `frontend/src/features/query/components/AppHeader.vue`, `frontend/src/features/query/components/QueryWorkspace.vue`, `frontend/src/features/person-detail/ratingTimelineGeometry.ts`, `frontend/src/app/App.vue`, `frontend/src/app/routes.ts`, `frontend/src/shared/charts/**`, `frontend/src/shared/components/AppIcon.vue`, `frontend/src/shared/components/SafeImage.vue`, `frontend/src/shared/components/DeferredSurfaceState.vue`, `frontend/src/shared/media/bangumiImage.ts`, `frontend/src/shared/styles/base.css`, `frontend/scripts/check-architecture.mjs`, `frontend/scripts/check-production-artifact.mjs`, `frontend/tests/api/candidates.test.ts`, `frontend/tests/api/partners.test.ts`, `frontend/tests/api/co-star.test.ts`, `frontend/tests/api/client.test.ts`, `frontend/tests/features/co-star/**`, `frontend/tests/features/query/coordinator.test.ts`, `frontend/tests/features/query/share-routes.test.ts`, `frontend/tests/features/query/components.test.ts`, `frontend/tests/app/app.mount.test.ts`, `frontend/tests/app/co-star.integration.test.ts`, `frontend/tests/shared/SafeImage.test.ts`, and this change's task markers. |
...
| Acceptance | Strict adapter/generator drift tests; coordinator cancellation/revision/latest-only tests including completion-time candidate and dependent-view replay during same-query collection refresh; compound child edits retain the complete latest view and publish no false prerequisite error; component/integration coverage for every topology and scope; production-artifact deferred-module 503→200 recovery proving a second network attempt; full frontend check; Light/Dark responsive browser verification at DESIGN breakpoints with keyboard/focus, reduced motion, overflow, console, and network checks. |
