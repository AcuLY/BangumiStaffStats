...
|---|---|
| Status | Investigated/specified/main-agent reviewed: complete. Group 1 is accepted at `9eae3216`, Group 2A at `f7233647`, and `expose-co-star` at `8b9d623a` with archive `a15073fb`. Tasks 2.6–2.7 may edit only their API/coordinator/feature/test paths; task 2.8 shared integration remains gated on the active oracle owner's commit and explicit main-agent handoff. Nothing is pushed, released, or deployed. |
...
- [ ] 2.8 After the active oracle owner commits and the main agent explicitly hands off every App/share/shared/check path, integrate selection-preserving query revision behavior, one-shot versioned share restore, App route with persistent `mode-panel-ranking` / `mode-panel-co-star` tabpanel ownership matching the Header tabs, responsive rail/Drawer/focus restoration, dual-theme feature CSS, shared chart/media primitives, architecture/artifact inventories, and oracle copy/hierarchy without adding a dependency or state/request layer.
