...
- [ ] 3.1 Audit full diff, synchronize accepted specs, run full applicable checks and exact-stage commits.
- [ ] 3.2 Push topic, review green PR and merge; obtain successful exact-revision linux/amd64 bundle.
- [ ] 3.3 Back up /etc/nginx/nginx.conf and /srv/bgmss-v2 state; verify rollback and no concurrent drift.
- [ ] 3.4 Deploy and switch only search.bgmss.fun application routes; verify /, /old/, /v2 compatibility, assets, API, health, metrics, browser and restart; preserve other services.
- [ ] 3.5 Record exact evidence and remaining independent limitations; archive only after requested acceptance completes.
