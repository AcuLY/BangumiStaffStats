...
- [ ] 1.3 In `backend/internal/runtimecache/{result_test.go,runtime_test.go}`, prove global cross-type cost/count/LRU behavior, acceptance above a hypothetical one-fifth share while within the per-item/global limits, immutable typed reads, wrong-type-first and conflicting-binding rejection, concurrent construction safety, one collection/negative/detached owner, one mixed-operation two-running/eight-queued executor without timing flakiness, and exact aggregate statistics.
