...
Superseded attempts retain the actual historical Harness revision that
produced each failed run rather than being relabelled as the final Harness.
Repository verification proves every such source is strictly after Product
and at or before final Harness on that lineage; Product, the later archive,
and side branches are rejected. Accepted and superseded run IDs remain
disjoint even when several failed runs share one historical source revision,
and the acceptance audit must include the exact Development Actions log
digest.
