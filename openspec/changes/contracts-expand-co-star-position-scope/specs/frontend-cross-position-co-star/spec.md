...
Status: specified/user-approved. Owner: frontend scope owner and primary, nonoverlapping sets in ../../design.md. Writable paths: those frontend sets. Read-only protected inputs: other changes/hunks and runtime data. Deletion complement: none. Mutable refs: none. Consumes: typed scope contracts/catalog. Produces: exact source and broad partner UI. Dependencies: contracts/backend. Deliverables: UI, coordination/recovery and tests. Acceptance: focused/full frontend tests, browser, build, diff checks. Non-goals: statistics or second Query owner. Operations deferred: all production. Stop/rollback conditions: stop conflicts, undo own hunks only.
