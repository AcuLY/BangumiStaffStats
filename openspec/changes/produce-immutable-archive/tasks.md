...
| Mutable refs | None. |
| Consumes | Accepted foundations/consumer, the corrected Archive contract and Contracts cases, and an explicit source/build request. |
| Produces | Indexed synthetic cases; then one inactive validated Archive version or bounded no-change/failure. |
| Dependencies | Apply blocked until `correct-archive-subject-semantics` has exited and `implement-backend-archive-consumer` plus pointer-free `backend/cmd/archive-smoke` are accepted. |
...
- [ ] 1.1 Preflight branch/HEAD/index and allowed dirty paths; verify strict-reviewed artifacts, the accepted/exited Archive subject correction, exact Contracts writable/protected inventories, and no overlap; stop before mutation on mismatch.
