...
| Mutable refs | None. |
| Consumes | The corrected and string-hardened closed Archive contract and SQLite v1 authority. |
| Produces | Closed, indexed synthetic producer inputs and expected results. |
| Dependencies | Accepted `contracts-archive-manifest` after exited `correct-archive-subject-semantics` and `harden-archive-manifest-string-semantics`; no runtime implementation dependency. |
