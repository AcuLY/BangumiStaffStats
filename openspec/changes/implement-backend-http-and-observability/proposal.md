...
|---|---|
| Status | investigated: complete; specified: approved; implemented: no; verified: targeted/all strict validation, doctor, and main-agent artifact review passed; committed/pushed/released/deployed: no |
...
| Deliverables | Server/router/middleware/transport, health/readiness, metrics/events, app assembly, architecture/inventory checks, documentation, and tests. |
| Acceptance | Exact route/method/media/body/error/request-ID tests, JSON fuzzing, cancellation/deadline/panic tests, Archive readiness integration, parseable low-cardinality metrics, event redaction/allowlist tests, full/race/vet/build/OpenSpec gates, and no residue. |
