...
The user requested replacing all non-BGMSS icons with the icon library
recommended by Naive UI. Current business icons are inline SVG in four
components, with duplicated drawings across features.

## What Changes

- INTENTIONAL_DELTA: replace business glyphs with xicons Ionicons 5 outlines,
  pinned `@vicons/ionicons5@0.13.0`, rendered directly as the library Vue SVG components.
- Keep semantic wrapper names and caller sizes/classes so controls, labels,
  tooltips, sorting direction and theme continue unchanged.
- Preserve the BGMSS brand image/favicon, data charts and Naive UI internal
  control glyphs. Charts are data visualizations, not application icons.
- PRESERVE_ORACLE outside this user-approved glyph delta against
...
- `frontend-design-system`: one vendor-backed business icon system with brand exception.
- `frontend-foundation`: exact approved icon runtime dependency.
