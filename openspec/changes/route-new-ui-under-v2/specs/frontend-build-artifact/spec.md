...
| Writable paths | `frontend/vite.config.ts`, `frontend/src/app/basePath.ts`, exact API/image mapping hunks in `frontend/src/api/client.ts` and `frontend/src/shared/media/bangumiImage.ts`, and exact related tests. |
