...
| Writable paths | This change; delta/main specs for the three named capabilities during sync; `frontend/vite.config.ts`; new `frontend/src/app/basePath.ts`; exact path-aware hunks in `frontend/src/app/routes.ts`, `frontend/src/features/query/share.ts`, `frontend/src/features/query/components/AppHeader.vue`, `frontend/src/api/client.ts`, and `frontend/src/shared/media/bangumiImage.ts`; exact related frontend tests; `operations/nginx/bgmss.conf`; `operations/README.md`; on `myserver`, a new exact admitted incoming bundle/release below `/srv/bgmss-v2`, existing transactional application refs/project through `operations/bin/deploy`, `/srv/bgmss-v2/config/nginx/nginx.conf`, `/etc/nginx/nginx.conf`, and one new exact pre-change Nginx backup. |
