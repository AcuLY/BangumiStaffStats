...
- 青色克制：只用于焦点、信息提示和人物 B；不得铺满大面积卡片。
...
- 候选栏需要搜索、浏览职位、已选/未选过滤和多职位选择。
- “浏览职位”只负责选人，不修改顶部已应用查询。
- 已选人物使用紧凑条目，只保留头像、主名称、职位与 A/B 标记；选择变化后分析结果自动刷新，不提供“查看分析”按钮。
...
- 青色：focus ring、信息 glyph、人物 B 的文字/描边/图表标记。
- 灰色：普通行、共同部分、额外人物/系列与非激活控件。
...
| metrics | 作品数、均分、综合分 | 该职位作品数、个人均分 |
| whole-row click | 聚焦人物 | 禁止承担选择 |
| action | disclosure | 独立 `aria-pressed` 按钮 |

Candidate variant 根节点使用 `<article>` / `<li>`，选择按钮放在内部。禁止在整行 `<button>` 内嵌套其他按钮或 select。
...
- 已选人物条目目标高度约 52–56px，只显示头像、主名称、职位、移除操作和 A/B 标记，不展示日文副名、作品数或 Person ID。
...
人物 A 使用粉色，人物 B 只使用青色文字或细描边。照片本身不得叠加大面积粉/青 wash。
...
- 图表背景使用 surface 或透明。
- 网格线使用 divider。
- 主系列使用 pink，人物 B 使用 teal；其余使用固定灰阶/固定调色板。
...
- 青色 focus ring 为 2px，并保留 2px offset。
