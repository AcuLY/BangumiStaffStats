...
Header Bar 使用独立的 92% chrome 与 blur(16px) saturate(135%)。正文内的 Query Summary 和展开后的 Query Editor 共同使用普通 surface；输入控件仍可使用 surface-raised。移动 Drawer 使用独立的 96% 偏灰表面，不与 Header 或正文查询表面混用。
