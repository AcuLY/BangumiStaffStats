...
require_root_layout() {
  ...
  [[ "$(stat -c '%u:%g:%a' -- "$root/data")" == "65532:65532:750" ]] ||
    die "data root must be 65532:65532 mode 0750"
  [[ -d "$root/data/versions" && ! -L "$root/data/versions" ]] ||
    die "Archive versions root must be a real directory"
  [[ "$(stat -c '%u:%g:%a' -- "$root/data/versions")" == "65532:65532:750" ]] ||
    die "Archive versions root must be 65532:65532 mode 0750"
  ...
  [[ "$(stat -c '%u:%g:%a' -- "$root/data/current.json")" == "65532:65532:640" ]] ||
    die "current data pointer must be 65532:65532 mode 0640"
}
...
verify_bundle() {
  ...
  for required in api.oci.tar frontend.tar \
    build.json compatibility-manifest.json minimal-archive/current.json \
...
verify_build_metadata() {
  local build_json=$1 revision api_image
  ...
  ' "$build_json" >/dev/null || die "bundle components must be Backend and Frontend"
}
...
release_env_document() {
  local root=$1 project=$2 api_image=$3 prom_image=$4 api_port=$5 prom_port=$6
  local profile=${7:-production}
...
release_topology_state() {
  ...
  cmp -s \
    <(
      release_env_document \
        "$root" "$project" "$current_api" \
        "$prom_image" "$api_port" "$prom_port" "$profile"
    ) \
    "$file" ||
    die "existing deployment topology/profile differs; deploy may only change the API image"
  printf 'clean\n'
}
