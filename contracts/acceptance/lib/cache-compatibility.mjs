...
export const CACHE_AUTHORITY_COUNT = 16;
...
function validateGoAuthority({
  ...
  if (
    ...
    fail('Go validation revision or go.sum binding is inconsistent');
  }
...
function validateAuthorityRecord(authority, expected, phase, index) {
  ...
  const hasPreparation = expected.logicalOwner === 'product';
...
function validatePhaseShape(phase, expectedName) {
  ...
  exactObject(
    document.counts,
    ['authorities', 'npmLocks', 'productLocks', 'goFiles', 'uvLocks'],
    `${expectedName} counts`,
  );
  ...
  if (
    ...
    fail(`${expectedName} authority set must contain exactly 16 records`);
  }
  exactObject(
    document.seals,
    [
