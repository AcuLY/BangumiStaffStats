...
describe('query shell components', () => {
  ...
  it('keeps unchanged submit silent and avoids duplicate footer pending copy', () => {
    ...
    expect(workspaceSource).toMatch(
      /!refreshCollection[\s\S]*?queryStore\.applied !== null[\s\S]*?!props\.queryStore\.dirty[\s\S]*?return;/,
    );
  ...
  it('keeps retained ranking results and one error copy when refresh fails in the editor', async () => {
    window.history.replaceState({}, '', '/ranking?user=luca');
    const pinia = createPinia();
    setActivePinia(pinia);
    validStore();
