...
describe('ranked person list', () => {
  ...
  it('shares compact one-line tracks and preserves the narrow two-line map', () => {
    ...
    expect(baseCss).toMatch(
      /@container ranking-pane \(max-width: 340px\)[\s\S]*?\.ranking-workspace \.ranked-person-row,\s*\.ranking-workspace \.ranking-row-skeleton\s*\{[^}]*grid-template-areas:\s*"rank avatar identity"\s*"\. metrics metrics";[^}]*grid-template-columns:\s*22px 36px minmax\(0, 1fr\);/,
    );
  });
});
...
describe('ranking result surface', () => {
  it.each([
    ...
    ({ pendingPersonal, pendingWorkUnit, metricCount, workLabel }) => {
      ...
      expect(wrapper.get('.ranking-columns__metrics').text()).toContain(
        workLabel,
      );
      expect(wrapper.find('.ranking-skeleton__column-label').exists()).toBe(
        false,
      );
      expect(
