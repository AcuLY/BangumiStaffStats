...
const temporaryRoot = path.join(frontendRoot, '.tmp/rankings-wire-committed');
const expectedFiles = ['schemas.gen.ts', 'types.gen.ts'];

if (process.version !== 'v24.18.0') {
  throw new Error(
    `rankings wire drift check requires Node v24.18.0, received ${process.version}`,
  );
}
...
try {
  const result = spawnSync(
    process.execPath,
    [path.join(frontendRoot, 'scripts/generate-rankings-wire.mjs')],
    {
      cwd: frontendRoot,
      env: process.env,
      stdio: 'inherit',
  ...
  assert.equal(result.status, 0, 'rankings generator failed');
  assert.deepEqual(inventory(generatedRoot), expectedFiles);
  for (const filename of expectedFiles) {
    assert(
      ...
      `generated rankings file drifted: ${filename}`,
    );
  }
  console.log('rankings wire drift check passed');
} finally {
