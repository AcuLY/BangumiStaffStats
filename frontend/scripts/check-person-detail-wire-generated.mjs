...
const temporaryRoot = path.join(
  frontendRoot,
  '.tmp/person-detail-wire-committed',
);
const expectedFiles = ['schemas.gen.ts', 'types.gen.ts'];

if (process.version !== 'v24.18.0') {
  throw new Error(
    `person-detail wire drift check requires Node v24.18.0, received ${process.version}`,
  );
}
...
try {
  const result = spawnSync(
    process.execPath,
    [path.join(frontendRoot, 'scripts/generate-person-detail-wire.mjs')],
    {
      cwd: frontendRoot,
      env: process.env,
      stdio: 'inherit',
  ...
  assert.equal(result.status, 0, 'person-detail generator failed');
  assert.deepEqual(inventory(generatedRoot), expectedFiles);
  for (const filename of expectedFiles) {
    assert(
      ...
      `generated person-detail file drifted: ${filename}`,
    );
  }
  console.log('person-detail wire drift check passed');
} finally {
