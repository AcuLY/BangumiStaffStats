...
const temporaryRoot = path.join(frontendRoot, '.tmp/co-star-wire-committed');
const expectedFiles = ['schemas.gen.ts', 'types.gen.ts'];

if (process.version !== 'v24.18.0') {
  throw new Error(
    `co-star wire drift check requires Node v24.18.0, received ${process.version}`,
  );
}
...
try {
  const result = spawnSync(
    process.execPath,
    [path.join(frontendRoot, 'scripts/generate-co-star-wire.mjs')],
    {
      cwd: frontendRoot,
      env: process.env,
      stdio: 'inherit',
  ...
  assert.equal(result.status, 0, 'co-star generator failed');
  assert.deepEqual(inventory(generatedRoot), expectedFiles);
  for (const filename of expectedFiles) {
    assert(
      ...
      `generated co-star file drifted: ${filename}`,
    );
  }
  console.log('co-star wire drift check passed');
} finally {
