...
- Go 后端后台 builder 负责每周离线构建，并在短维护窗口内热替换 Store；不依赖 systemd、Python updater 或进程重启。
- 成功替换后只保留当前不可变快照、manifest、目录/规则和有界运维制品；收藏、查询结果、图片和会话不得持久化。
- v1 动态暴露五类全部 common exact 职位，并额外提供动画/游戏 main/all 声优。
- v1 使用单实例内的原子 Store/指针热切换；构建阶段继续服务。激活阶段暂停新查询、排空在途任务、释放旧派生缓存后预热候选，再提交 Store/指针；不同时保留两代完整缓存。失败时保留并恢复旧数据，重新预热成功后恢复查询；恢复失败保持未就绪。
