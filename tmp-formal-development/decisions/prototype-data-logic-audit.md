...
### 声优角色范围修订（2026-09-12 用户确认）

本修订取代前文仅开放 main/all 的范围限制，保留全部 exact/valid_cv 来源口径、统计语义和同类型声优范围互斥规则。动画和游戏各开放 main=1、supporting=2、guest=3、minor=4、narrator=5、voice-library=6、all=1..6。单独职位分别标为声优（主役）、声优（配角）、声优（客串）、声优（闲角）、声优（旁白）、声优（声库），全部职位保持“声优”；所有名称去掉“仅”。角色标签直接展示对应真实类型，不再将 4–6 显示为“其他”。全部职位浏览仅自动加入 all，显式单独类型身份不被替换。
