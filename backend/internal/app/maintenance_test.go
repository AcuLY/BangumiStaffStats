...
func TestActivateCandidateSwapsWithoutListenerRestartAndCleansOld(t *testing.T) {
	...
	copyArchiveTestFile(
		t,
		filepath.Join(oldVersionRoot, "manifest.json"),
		filepath.Join(newVersionRoot, "manifest.json"),
	)
	...
	candidate, err := archive.OpenCandidate(context.Background(), root, newVersion)
	if err != nil {
		t.Fatal(err)
	}
...
func TestActivateCandidatePointerFailureRestoresOldStore(t *testing.T) {
	...
	candidate, err := archive.OpenCandidate(
		context.Background(),
		root,
