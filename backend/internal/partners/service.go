...
// NewService constructs an empty production partner service.
func NewService(
	stores StoreProvider,
	collections CollectionProvider,
	config Config,
	...
	if err != nil {
		return nil, fmt.Errorf("partners: create executor: %w", err)
	}
	collectionCache, err := runtimecache.NewCollectionCache(config.Collection)
	if err != nil {
		return nil, fmt.Errorf("partners: create collection cache: %w", err)
	}
