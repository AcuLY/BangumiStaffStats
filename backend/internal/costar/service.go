...
// NewService constructs an empty production co-star service.
func NewService(
	stores StoreProvider,
	collections CollectionProvider,
	config Config,
	...
	if err != nil {
		return nil, fmt.Errorf("costar: create executor: %w", err)
	}
	collectionCache, err := runtimecache.NewCollectionCache(config.Collection)
	if err != nil {
		return nil, fmt.Errorf("costar: create collection cache: %w", err)
	}
