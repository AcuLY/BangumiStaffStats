...
work_root="$temporary_root/co-star-wire"
tool_root="$work_root/tool"
projection_root="$work_root/projection"
generated_file="$work_root/co_star.gen.go"
target_file="$backend_root/internal/httpapi/wire/co_star.gen.go"
...
cp \
  "$repository_root/contracts/goldens/api/co-star/package.json" \
  "$repository_root/contracts/goldens/api/co-star/package-lock.json" \
  "$tool_root/"
(
  cd "$tool_root"
...
"$node_command" "$script_root/prepare-co-star-wire.mjs" "$projection_root"
...
"$node_command" "$redocly_cli" bundle \
  ...
  --output "$work_root/co-star.bundle.json" \
...
(
  ...
  "$go_command" tool oapi-codegen \
    ...
    -o "$generated_file" \
    "$work_root/co-star.bundle.json"
)
"$pinned_gofmt" -w "$generated_file"
for declaration in \
  PostCoStarV1JSONBody \
  PostCoStarV1200JSONResponseBody \
  PostCoStarV1400JSONResponseBodyErrorCode; do
  if ! grep -q "^type ${declaration} " "$generated_file"; then
    echo "generated co-star model is missing $declaration" >&2
...
echo "co-star wire generation is current"
