...
work_root="$temporary_root/candidates-wire"
tool_root="$work_root/tool"
projection_root="$work_root/projection"
generated_file="$work_root/candidates.gen.go"
target_file="$backend_root/internal/httpapi/wire/candidates.gen.go"
...
pinned_gofmt="$("$go_command" env GOROOT)/bin/gofmt"

cp \
  "$repository_root/contracts/goldens/api/candidates/package.json" \
  "$repository_root/contracts/goldens/api/candidates/package-lock.json" \
  "$tool_root/"
(
  cd "$tool_root"
...
"$node_command" "$script_root/prepare-candidates-wire.mjs" "$projection_root"
...
"$node_command" "$redocly_cli" bundle \
  ...
  --output "$work_root/candidates.bundle.json" \
...
(
  ...
  "$go_command" tool oapi-codegen \
    ...
    -o "$generated_file" \
    "$work_root/candidates.bundle.json"
)
"$pinned_gofmt" -w "$generated_file"
for declaration in \
  PostCandidatesV1JSONBody \
  PostCandidatesV1200JSONResponseBody \
  PostCandidatesV1400JSONResponseBodyErrorCode; do
  if ! grep -q "^type ${declaration} " "$generated_file"; then
    echo "generated candidates model is missing $declaration" >&2
...
if [[ ! -f "$target_file" ]] || ! cmp -s "$generated_file" "$target_file"; then
  echo "generated candidates model is stale: ${target_file#"$backend_root/"}" >&2
  exit 1
fi
echo "candidates wire generation is current"
