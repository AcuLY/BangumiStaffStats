...
work_root="$temporary_root/partners-wire"
tool_root="$work_root/tool"
projection_root="$work_root/projection"
generated_file="$work_root/partners.gen.go"
target_file="$backend_root/internal/httpapi/wire/partners.gen.go"
...
cp \
  "$repository_root/contracts/goldens/api/partners/package.json" \
  "$repository_root/contracts/goldens/api/partners/package-lock.json" \
  "$tool_root/"
(
  cd "$tool_root"
...
"$node_command" "$script_root/prepare-partners-wire.mjs" "$projection_root"
...
"$node_command" "$redocly_cli" bundle \
  ...
  --output "$work_root/partners.bundle.json" \
...
(
  ...
  "$go_command" tool oapi-codegen \
    ...
    -o "$generated_file" \
    "$work_root/partners.bundle.json"
)
"$pinned_gofmt" -w "$generated_file"
for declaration in \
  PostPartnersV1JSONBody \
  PostPartnersV1200JSONResponseBody \
  PostPartnersV1400JSONResponseBodyErrorCode; do
  if ! grep -q "^type ${declaration} " "$generated_file"; then
    echo "generated partners model is missing $declaration" >&2
...
if [[ ! -f "$target_file" ]] || ! cmp -s "$generated_file" "$target_file"; then
  echo "generated partners model is stale: ${target_file#"$backend_root/"}" >&2
  exit 1
fi
echo "partners wire generation is current"
