...
const allowedRoot = path.join(backendRoot, ".tmp", "candidates-wire", "projection");
...
if (typeof targetArgument !== "string" || process.argv.length !== 3) {
  throw new Error("usage: node prepare-candidates-wire.mjs <projection-root>");
}
const projectionRoot = path.resolve(targetArgument);
assert.equal(projectionRoot, allowedRoot, "unexpected candidates projection root");
