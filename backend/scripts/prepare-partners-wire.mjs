...
const allowedRoots = new Set([
  path.join(backendRoot, ".tmp", "partners-wire", "projection"),
  path.join(repositoryRoot, "frontend", ".tmp", "partners-wire", "projection"),
]);
const partnersInfo = {
...
if (typeof targetArgument !== "string" || process.argv.length !== 3) {
  throw new Error("usage: node prepare-partners-wire.mjs <projection-root>");
}
const projectionRoot = path.resolve(targetArgument);
assert(allowedRoots.has(projectionRoot), "unexpected partners projection root");
...
const operation = authority.paths["/partners"]?.post;
assert.equal(operation?.operationId, "postPartnersV1");
const selected = collectLocalComponents(authority, authority.paths["/partners"]);
