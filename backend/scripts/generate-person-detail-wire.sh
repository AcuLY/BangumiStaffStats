...
work_root="$temporary_root/person-detail-wire"
tool_root="$work_root/tool"
projection_root="$work_root/projection"
generated_file="$work_root/person_detail.gen.go"
target_file="$backend_root/internal/httpapi/wire/person_detail.gen.go"
...
cp \
  "$repository_root/contracts/goldens/api/person-detail/package.json" \
  "$repository_root/contracts/goldens/api/person-detail/package-lock.json" \
  "$tool_root/"
(
  cd "$tool_root"
...
"$node_command" "$script_root/prepare-person-detail-wire.mjs" "$projection_root"
...
"$node_command" "$redocly_cli" bundle \
  ...
  --output "$work_root/person-detail.bundle.json" \
  --config "$projection_root/redocly.yaml"
(
  cd "$backend_root"
  "$go_command" tool oapi-codegen \
    -config "$projection_root/oapi-codegen.yaml" \
    -o "$generated_file" \
    "$work_root/person-detail.bundle.json"
)
"$pinned_gofmt" -w "$generated_file"
for declaration in \
  PostPersonDetailV1JSONBody \
  PostPersonDetailV1200JSONResponseBody \
  PostPersonDetailV1400JSONResponseBodyErrorCode; do
  if ! grep -q "^type ${declaration} " "$generated_file"; then
    echo "generated person-detail model is missing $declaration" >&2
...
if [[ ! -f "$target_file" ]] || ! cmp -s "$generated_file" "$target_file"; then
  echo "generated person-detail model is stale: ${target_file#"$backend_root/"}" >&2
  exit 1
fi
echo "person-detail wire generation is current"
